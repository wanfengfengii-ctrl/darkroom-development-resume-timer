...
- **提前结束本阶段**：试片密度已达标或需立即换液时，运行/暂停中都可从计时面板提前结束当前药浴。操作携带点击时面板显示的阶段；状态机先按当前墙钟消费自然到期时间（当前阶段已到期却尚未由 tick 推进时先自然跨阶段），**仅阶段仍一致才转入下一药浴**——运行态从下一阶段完整时长倒数，暂停态进入下一阶段后仍暂停并保存完整时长；跳过显影同步清除搅动提示，跳过定影直接进入完成结果。若阶段已在另一标签页或计时推进中变化，本次操作不越过新阶段：界面采纳较高修订记录并提示「阶段已更新，未提前结束」，避免陈旧面板误跳两段。成功操作属显式操作（rev +1），经 localStorage 提交与 storage 事件同步到其它标签页。
...
- 显式操作（开始/暂停/继续/校准/确认搅动/结束本阶段/重置）把 rev +1；其余标签页收到更高 rev 立即跟进（例如在 B 标签暂停，A 标签也进入暂停）。
...
npm test           # Vitest：状态机转换（156 个用例）
npm run e2e        # Playwright：自动 build + preview 后跑验收（43 个用例）
...
```
src/
  timer/engine.ts      纯函数状态机：校验/启动/推进跨阶段/暂停/继续/校准/结束本阶段/回拨锁定/持久化
  timer/engine.test.ts Vitest 状态转换测试
  timer/useTimer.ts    React 绑定：tick、可见性恢复、localStorage 读写
  components/          配方表单、计时面板（阶段提示/倒计时/校准/结束本阶段/完成/锁定）
e2e/app.spec.ts        Playwright 验收：刷新、息屏逾期跨阶段、后台、暂停、校准、提前结束、回拨
